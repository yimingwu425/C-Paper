...
- **status**: Completed
- **log**: 2026-06-06: RED: `swift test --jobs 1 --filter SupportDiagnosticsTests` initially failed at compile time because `SupportDiagnostic` / `SupportDiagnosticsStore` did not exist. GREEN: added a redacted support diagnostic model and `SupportDiagnosticsStore` that writes `Support/latest-diagnostic.txt`; wired startup, source-provider warnings/failures, download failures, preview failures, and update failures to create copyable diagnostics; added alert, downloads view, settings, and startup failure actions to copy diagnostics or reveal the support folder. Sensitive proxy credentials, EasyPaper tokens, query secrets, and private home paths are redacted before display or file write. Validation PASS: `swift test --jobs 1 --filter SupportDiagnosticsTests`, `swift test --jobs 1 --filter ModelTests/testBackendErrorsCreateRedactedSupportDiagnosticReport`, `swift test --jobs 1 --filter StartupBootCoordinatorTests`, and full `swift test --jobs 1` passed with 78 tests, 4 skipped, and 0 failures.
- **files edited/created**: `macos/Sources/CPaperNativeApp/Models/SupportDiagnostics.swift`; `macos/Sources/CPaperNativeApp/Backend/Persistence/SupportDiagnosticsStore.swift`; `macos/Sources/CPaperNativeApp/Backend/Core/NativeBackendService.swift`; `macos/Sources/CPaperNativeApp/State/AppBootCoordinator.swift`; `macos/Sources/CPaperNativeApp/State/AppModel.swift`; `macos/Sources/CPaperNativeApp/State/AppModel+Setup.swift`; `macos/Sources/CPaperNativeApp/State/AppModel+PaperWorkflow.swift`; `macos/Sources/CPaperNativeApp/State/AppModel+Updates.swift`; `macos/Sources/CPaperNativeApp/Views/RootView.swift`; `macos/Sources/CPaperNativeApp/Views/DownloadsView.swift`; `macos/Sources/CPaperNativeApp/Views/PDFPreviewView.swift`; `macos/Sources/CPaperNativeApp/Views/SettingsView.swift`; `macos/Sources/CPaperNativeApp/Views/SettingsInfoSections.swift`; `macos/Tests/CPaperNativeTests/SupportDiagnosticsTests.swift`; `macos/Tests/CPaperNativeTests/ModelTests.swift`; `macos/Tests/CPaperNativeTests/StartupBootCoordinatorTests.swift`; `cpaper-professionalization-plan.md`; `docs/WORK_LOG.md`
