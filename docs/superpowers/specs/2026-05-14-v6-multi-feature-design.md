...
- **移动端**：iOS 原生 App（SwiftUI + iOS 18 液态玻璃风格）
- **协作联机**：用户系统 + 分享 + 学习小组 + 试卷评价
- **智能化**：AI 试卷分析、OCR 题目提取、智能去重

### 1.2 目标

1. **iOS App** — SwiftUI 原生应用，iOS 18+，液态玻璃设计语言
...
### 3.1 技术选型（仅 iOS，不做 Android）

| 维度 | 选择 | 说明 |
|---|---|---|
| UI | SwiftUI（纯原生） | 声明式 UI，iOS 18+ `@Observable` 宏 |
| 视觉效果 | `.glassEffect` / `GlassEffect` | iOS 18 原生液态玻璃风格 |
| 网络 | async/await + URLSession | 系统原生，零依赖 |
| 持久化 | SwiftData | 系统原生 ORM，自动 iCloud 同步 |
| PDF 渲染 | PDFKit | 系统原生 |
| OCR | Vision Framework (`VNRecognizeTextRequest`) | 系统原生，英文识别准确率 >95% |
| 认证 | ASWebAuthenticationSession + AuthenticationServices | Apple Sign In 内建，飞书 OAuth 走 WebView |
| 最低版本 | iOS 18.0 | 需要 `GlassEffect` API |
...
| 文件预览 | 内嵌 PDFKit 查看器 |
| 收藏管理 | 通过协作服务端云端同步 |
| 主题切换 | 系统自动（跟随 iOS 明/暗模式） |
| 账号登录 | 邮箱 + Apple Sign In + 飞书 OAuth |
| 分享链接 | 创建/打开分享 |
| 学习小组 | 创建/加入/查看，SSE 实时进度 |
| 试卷评价 | 评分+评语 |
| AI 分析 | 本地 AI 分析（需配 API key，通过 LLM API） |
| OCR 提取 | Vision Framework 本地 OCR |
| 智能去重 | Core ML embedding 模型 + 本地向量相似度 |

### 3.3 UI 设计原则

- 底部 Tab Bar（搜索、小组、收藏、我的）— 液态玻璃材质背景
- NavigationStack 导航，工具栏按钮使用 `.glassEffect`
- 列表/卡片使用 `.glassEffect` 或半透明材料
- 搜索栏、下载进度条融入液态玻璃风格
- 下拉刷新（RefreshControl）+ 无限滚动
- 原生手势支持（滑动删除、长按预览）
- 适配 iOS 18 控件风格（TabView 新样式、sfSymbols 动画）
- 支持 Dynamic Island 展示下载进度
...
| 目录 | 内容 |
|---|---|
| `ios/` | SwiftUI 项目（Xcode project，iOS 18+ target） |
| `docs/mobile/` | 移动端开发文档 |
...
|---|---|
| 搜索/下载 | 功能与桌面端一致 |
| 登录 | Apple Sign In + 飞书 OAuth 流程正常 |
| 分享/小组 | UI 正确展示，SSE 实时更新 |
| 离线 | 无网络时本地功能可用 |
| 液态玻璃 | `.glassEffect` 材料在明/暗模式下均正确渲染 |
...
iOS:
  SwiftUI (built-in), PDFKit (built-in), Vision (built-in OCR), 
  SwiftData (built-in), CryptoKit (built-in), Observation (built-in)
```
...
| 移动端开发量大 | 仅做 iOS，不做 Android |
| embedding 模型大 | 使用 tiny 模型（80MB），首次下载时提示 |
| iOS 18 最低版本限制 | 液态玻璃是 iOS 18+ 特性，部分用户需升级系统 |
